...
void generate_print(CodeGenerator *generator, AstNode *node) {
    ...
    for (i = 0; i < node->data.function_call.args->size; i++) {
        ...
        if (curr_arg_expr->contains_variables) {
            ...
            switch (curr_arg_expr->value->type) {
                case TYPE_BOOL: // TODO: implement bool printing to print "true" and "false"
                case TYPE_DOUBLE:
                case TYPE_INT:
                    ...
                    break;
                case TYPE_CHAR:
                    write_to_file(generator->fp, PUSH, "1"); // buf
                    write_to_file(generator->fp, PUSH, curr_arg_expr->value->value.double_value); // count
                    write_to_file(generator->fp, CALL, PRINT_PROC);
                    break;
                case TYPE_STRING:
